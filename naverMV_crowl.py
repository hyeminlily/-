...
def getMovieList():
    ...
    for t in title_list:
        ...
        if html.find('ul', {'class': "search_list_1"}) != None:
            ...
            if html_mv.find('div', {'class': "mv_info"}) != None:
                info = html_mv.find('div', {'class': "mv_info"})

                # get title, titleEng
                title = ''
                if info.find('h3', {'class': "h_movie"}) != None:
                    title = info.find('h3', {'class': "h_movie"}).find('a').text
                movie.append(title)

                titleEng = ''
                if info.find('strong', {'class': "h_movie2"}) != None:
            ...
            # get director list
            if html_dt.findAll('div', {'class': "dir_product"}) != None:
            ...
            # get actor list
            if html_dt.findAll('div', {'class': "p_info"}) != None:
            ...
            # get poster image
            if html_mv.find('div', {'class': "poster"}) != None:
            ...
            # get content
            if html_mv.find('div', {'class': 'story_area'}) != None:
